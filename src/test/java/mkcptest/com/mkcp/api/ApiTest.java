package mkcptest.com.mkcp.api;

import com.alibaba.fastjson.JSON;
import com.mkcp.api.beans.ApiDataBean;
import com.mkcp.api.configs.ApiConfig;
import com.mkcp.api.listeners.AutoTestListener;
import com.mkcp.api.listeners.RetryListener;
import com.mkcp.api.utils.*;
...
public class ApiTest extends TestBase {
    ...
    @BeforeTest
    public void readData() throws DocumentException {
        dataList = JDBCUtil.ApiDataBean();
    }
    ...
    public void apiTest(ApiDataBean apiDataBean) throws Exception {
        ...
        try {
            ...
            if (tokens != null && tokens.length > 0) {
                TOKEN = tokens[0].toString().replace("token:","");
            }
...
//						.toString(respEntity));
    ...
    private HttpUriRequest parseHttpRequest(String url, String method, String param, String paramType) throws UnsupportedEncodingException {
        ...
        if ("post".equalsIgnoreCase(method)) {
            ...
            if (StringUtils.isNotBlank(TOKEN)) {
                putMethod.addHeader("token", TOKEN);
            }
            if (StringUtils.isNotBlank(paramType)) {
            ...
            if (StringUtils.isNotBlank(TOKEN)) {
                deleteMethod.addHeader("token", TOKEN);
            }
            ...
            if (StringUtils.isNotBlank(TOKEN)) {
                getMethod.addHeader("token", TOKEN);
            }
            if (StringUtils.isNotBlank(paramType)) {
