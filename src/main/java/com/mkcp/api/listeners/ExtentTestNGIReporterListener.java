package com.mkcp.api.listeners;
...
import com.mkcp.api.utils.ReportUtil;
import org.testng.*;
...
public class ExtentTestNGIReporterListener implements IReporter {
  ...
  //生成的路径以及文件名
  private static final String OUTPUT_FOLDER = "D://JENKINS_HOME//workspace//mkcp-api-test/";
  ...
  public void generateReport(List<XmlSuite> xmlSuites, List<ISuite> suites,
    ...
    for (ISuite suite : suites) {
      ...
      if (suiteTest != null) {
        suiteTest.getModel().setDescription(String
            .format("Pass: %s ; Fail: %s ; Skip: %s ;", suitePassSize, suiteFailSize,
                suiteSkipSize));
    ...
    if (tests.size() > 0) {
      ...
      for (ITestResult result : treeSet) {
        ...
        for (Object param : parameters) {
          String params = param.toString().split(",")[0];
          name += params.split(":")[1];
        }
        if (name.length() > 0) {
          if (name.length() > 50) {
        ...
        while (logIterator.hasNext()) {
          ...
          if (details.contains(ReportUtil.getSpiltTimeAndMsg())) {
            ...
            log.setDetails(
                details.substring(time.length() + ReportUtil.getSpiltTimeAndMsg().length()));
          } else {
